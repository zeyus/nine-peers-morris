...
describe('NinePeersMorris', () => {
	...
	beforeEach(() => {
		...
		mockWindow = createMockWindow() as Window;
	...
	describe('piece placement', () => {
		...
		it('should detect mill formation and enter capture phase', () => {
			...
			game.handleCellClick(game.getBoard.getCell(2)); // p1
	...
	describe('game flow', () => {
		it('should transition from placement to movement phase', () => {
			...
			const allPiecesPlaced = game.getPlayers.every((player) => player.unplacedPieces.length === 0);
	...
	describe('flying rules', () => {
		...
		it('should allow flying when player has 3 or fewer pieces', () => {
			const fromCell = game.getBoard.getCell(0);
			game.getBoard.getCell(23); // Far away cell
	...
	describe('getters', () => {
		it('should return readonly arrays', () => {
			...
			expect(() => {
				(players as Player[]).push(new Player('test', 'Test', false));
			}).toThrow();
		});
