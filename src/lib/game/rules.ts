...
export class GameRules {
	...
	static getRemovablePieces(opponent: Player, board: Board): readonly GamePiece[] {
		...
		const piecesNotInMill = opponent.placedPieces.filter(
			(piece) => !this.isMillFormed(piece.cell!, board)
		);
	...
	static getNextPhase(currentPhase: GamePhase, players: readonly Player[]): GamePhase {
		if (currentPhase === GamePhase.Placement) {
			...
			const allPiecesPlaced = players.every((player) => player.unplacedPieces.length === 0);
			if (allPiecesPlaced) {
